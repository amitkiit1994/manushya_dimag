...
class Tenant(Base):
    ...
    # Relationships
    identities: Mapped[list["Identity"]] = relationship("Identity", back_populates="tenant")
    ...
    identity_events: Mapped[list["IdentityEvent"]] = relationship("IdentityEvent", back_populates="tenant")
    rate_limits: Mapped[list["RateLimit"]] = relationship("RateLimit", back_populates="tenant")
    webhooks: Mapped[list["Webhook"]] = relationship("Webhook", back_populates="tenant")
    webhook_deliveries: Mapped[list["WebhookDelivery"]] = relationship("WebhookDelivery", back_populates="tenant")
...
class Identity(Base):
    ...
    tenant: Mapped["Tenant | None"] = relationship("Tenant", back_populates="identities")
    # SSO fields
    sso_provider: Mapped[str | None] = mapped_column(String(50), nullable=True, index=True)
    sso_external_id: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    sso_email: Mapped[str | None] = mapped_column(String(255), nullable=True, index=True)
    ...
    sent_invitations: Mapped[list["Invitation"]] = relationship("Invitation", back_populates="invited_by_identity")
    sessions: Mapped[list["Session"]] = relationship("Session", back_populates="identity")
    events: Mapped[list["IdentityEvent"]] = relationship("IdentityEvent", back_populates="identity", foreign_keys="[IdentityEvent.identity_id]")
    acted_events: Mapped[list["IdentityEvent"]] = relationship("IdentityEvent", back_populates="actor", foreign_keys="[IdentityEvent.actor_id]")
...
class Invitation(Base):
    ...
    # Relationships
    invited_by_identity: Mapped["Identity | None"] = relationship("Identity", back_populates="sent_invitations")
...
class IdentityEvent(Base):
    ...
    tenant_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tenants.id", ondelete="SET NULL"), nullable=True)
    tenant: Mapped["Tenant | None"] = relationship("Tenant", back_populates="identity_events")

    # Relationships
    identity: Mapped["Identity | None"] = relationship("Identity", back_populates="events", foreign_keys=[identity_id])
    actor: Mapped["Identity | None"] = relationship("Identity", back_populates="acted_events", foreign_keys=[actor_id])
...
class RateLimit(Base):
    ...
    tenant_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tenants.id", ondelete="SET NULL"), nullable=True)
    tenant: Mapped["Tenant | None"] = relationship("Tenant", back_populates="rate_limits")

    # Constraints
    __table_args__ = (
        Index("idx_rate_limits_client_endpoint_window", "client_key", "endpoint", "window_start"),
...
class Webhook(Base):
    ...
    # Relationships
    created_by_identity: Mapped["Identity | None"] = relationship("Identity", foreign_keys=[created_by])
...
class WebhookDelivery(Base):
    ...
    tenant_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("tenants.id", ondelete="SET NULL"), nullable=True)
    tenant: Mapped["Tenant | None"] = relationship("Tenant", back_populates="webhook_deliveries")
