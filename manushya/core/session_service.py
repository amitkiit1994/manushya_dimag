...
class SessionService:
    ...
    async def revoke_all_sessions_for_identity(
        db: AsyncSession, identity_id: str, except_session_id: str | None = None
    ) -> int:
    ...
    async def get_active_sessions_for_identity(
        ...
        stmt = (
            select(Session)
            .where(Session.identity_id == identity_id, Session.is_active)
    ...
    async def cleanup_expired_sessions(db: AsyncSession) -> int:
        ...
        stmt = select(Session).where(
            Session.expires_at < datetime.now(UTC), Session.is_active
        )
        result = await db.execute(stmt)
