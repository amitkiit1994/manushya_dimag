...
async def create_api_key(
    ...
    try:
        ...
        if not tenant_id:
            raise HTTPException(
                status_code=400, detail="Identity must belong to a tenant"
            )
        # Generate API key
        ...
        api_key = ApiKey(
            ...
            is_active=True,
            expires_at=(
                datetime.utcnow() + timedelta(days=api_key_data.expires_in_days)
        ...
        return ApiKeyCreateResponse(
            api_key=ApiKeyResponse.from_orm(api_key), secret_key=api_key_value
        )
