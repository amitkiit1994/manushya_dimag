...
class WebhookUpdate(BaseModel):
    name: str | None = Field(
        None, min_length=1, max_length=255, description="Webhook name"
    )
...
@router.post(
    "/{webhook_id}/deliveries/{delivery_id}/retry", status_code=status.HTTP_200_OK
)
async def retry_webhook_delivery(
    ...
    return {
        ...
        "message": (
            ...
            if success
            else "Webhook delivery retry failed"
        ),
    }
...
async def get_webhook_stats(
    ...
    active_webhook_stmt = select(func.count(Webhook.id)).where(
        Webhook.tenant_id == tenant_id, Webhook.is_active
    )
