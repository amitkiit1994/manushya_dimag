...
async def sso_login(provider: str, request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"SSO login failed: {str(e)}"
        ) from e
...
async def sso_callback(
    ...
    try:
        ...
        if not sso_external_id:
            raise HTTPException(
                status_code=400, detail="Invalid SSO response: missing user ID"
            )
        ...
        if identity:
            ...
            identity = Identity(
                ...
                claims={"sso_provider": provider, "name": name, "email": email},
        ...
        raise HTTPException(
            status_code=400, detail=f"SSO callback failed: {str(e)}"
        ) from e
