...
class BulkDeleteRequest(BaseModel):
    identity_ids: list[uuid.UUID] = Field(..., description="List of identity IDs to delete")
...
async def get_identity(
    ...
    """Get identity by ID (requires appropriate permissions). Only for current tenant unless global/system-level."""
    # Check permissions
    policy_engine = PolicyEngine(db)
    await policy_engine.check_identity_access(
        current_identity, "read", target_role=None
    )
...
async def list_identities(
    ...
    limit: int = 100,
    role: str | None = None,
    is_active: bool | None = Query(None, description="Filter by active/inactive identities. Default: only active."),
    current_identity: Identity = Depends(get_current_identity),
    db: AsyncSession = Depends(get_db),
):
    """List identities (requires appropriate permissions).
    By default, only active identities are returned. Use is_active=false to list inactive, or is_active=all to list all (admin only).
    Only returns identities for the current tenant, unless current identity is global/system-level.
    """
    # Check permissions
    policy_engine = PolicyEngine(db)
    await policy_engine.check_identity_access(
        current_identity, "list", target_role=None
    )
...
async def delete_identity(
    ...
    policy_engine = PolicyEngine(db)
    await policy_engine.check_identity_access(
        current_identity, "delete", target_role=None
    )
    ...
    for identity_id in delete_request.identity_ids:
        try:
            ...
            if delete_request.hard_delete:
                ...
                if related_memories:
                    # Delete related memories first to avoid foreign key constraint issues
