            ...
            async with AsyncSessionLocal() as session:
                ...
                stmt = select(Memory).where(
                    and_(
                        ...
                        Memory.created_at < datetime.utcnow() - timedelta(days=Memory.ttl_days)
                    )
                )
                result = await session.execute(stmt)
...
def reindex_memories_task(self, memory_ids: Optional[list] = None):
...
def create_memory_embedding_task_async(memory_id: str):
    """Create a task to generate embedding for a memory."""
    return create_memory_embedding_task.delay(str(memory_id))


def batch_create_embeddings_task(memory_ids: list):
    """Create a task to generate embeddings for multiple memories."""
    return batch_create_embeddings_task.delay(memory_ids)


def cleanup_expired_memories_task():
    """Create a task to cleanup expired memories."""
    return cleanup_expired_memories_task.delay()


def reindex_memories_task(memory_ids: Optional[list] = None):
    """Create a task to reindex memories."""
    return reindex_memories_task.delay(memory_ids) 